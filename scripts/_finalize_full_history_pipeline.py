...
def replace_once(path: Path, old: str, new: str) -> None:
    ...
    if count != 1:
        raise SystemExit(
            f"expected one replacement in {path}: found {count}; marker={old[:120]!r}"
        )
    path.write_text(text.replace(old, new, 1), encoding="utf-8")


def patch_materializer() -> None:
    path = Path("scripts/materialize_partition.py")
    replace_once(
        path,
...
'''
    ...
    if text.count(marker) != 1:
        raise SystemExit("materializer helper insertion marker mismatch")
...
def patch_release_workflow() -> None:
    path = Path(".github/workflows/build-full-history-release.yml")
    replace_once(path, "  cancel-in-progress: false\n", "  cancel-in-progress: true\n")

    old_commit_arg = '            --repository-commit "$GITHUB_SHA"\n'
    continuation = "\\"
    new_commit_arg = (
        f'            --repository-commit "$GITHUB_SHA" {continuation}\n'
        "            --resample-workers 4\n"
    )
    replace_once(path, old_commit_arg, new_commit_arg)
    ...
    replace_once(
        ...
        "          if repository_commits != [os.environ[\"GITHUB_SHA\"]]:\n"
        "              raise SystemExit(\n"
        "                  f\"partition commit mismatch: expected={os.environ['GITHUB_SHA']}, \"\n"
    ...
    old_publish = '          gh release edit "$RELEASE_TAG" --draft=false --latest=false\n'
    publish = (
        f'          gh release edit "$RELEASE_TAG" {continuation}\n'
        f'            --target "$GITHUB_SHA" {continuation}\n'
        f"            --draft=false {continuation}\n"
        "            --latest=false\n"
    )
    replace_once(path, old_publish, publish)


def write_contract_test() -> None:
    Path("tests/test_materialize_partition.py").write_text(
        '''from __future__ import annotations
...
from scripts.materialize_partition import resample_partition
